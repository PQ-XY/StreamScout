...
const renderCardTitle = (item, loggedIn, favs = [], favOnChange) => {
  ...
  const favOnClick = () => {
    console.log("Trying to add/remove favorite:", item);
    if (isFav) {
    ...
    <>
      {
        ...
        <Tooltip title={isFav ? "Remove from favorite list" : "Add to favorite list"}>
          <Button shape="circle" style={{ marginBottom:10}} icon={isFav ? <StarFilled /> : <StarOutlined />} onClick={favOnClick} />
        </Tooltip>
      }
    ...
    <List
      ...
      renderItem={item => (
        <List.Item style={{ marginRight: '20px' }}>       
                ...
                >
                    <Meta title={renderCardTitle(item, loggedIn, favs, favOnChange)}></Meta>
                </Card>
        </List.Item>
      )}
    />
